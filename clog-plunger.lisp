...
(defun make-new-card (board)
  (let ((card (create-index-card board)))
...
(defun on-file-free (obj)
  (let* ((app (connection-data-item obj "app-data"))
    ...
    (setf (background-attachment (window-content win)) :local)
    (set-on-click (create-button (window-content win) :content "Add Card")
      (lambda (obj)
        (declare (ignore obj))
        (make-new-card (window-content win))))))
